  ...
  const drawPaths = (context, width, height, randomness) => {
    ...
    for (let i = 0; i <= parameters.lines; i++) {
      ...
      for (let x = 0; x <= width; x++) {
        ...
        context.lineTo(
          x,
          height / 2 + (parameters.amplitude + Math.random() * 0.005) * randomY
        );
      }
      const alpha = Math.min(Math.abs(randomY) + 0.001, 0.3);
      context.strokeStyle = `rgba(${parameters.waveColor.r}, ${
        parameters.waveColor.g
      }, ${parameters.waveColor.b}, ${alpha * 2})`;
      context.stroke();
      context.closePath();
